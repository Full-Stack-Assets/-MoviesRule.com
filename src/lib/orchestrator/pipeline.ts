...
export interface TopicPipelineOptions {
  /** If true, don't commit to GitHub — return the MDX content instead. */
  dryRun?: boolean;
  /** Frontmatter date for the post (defaults to now). The backfill runner spreads
   *  these across recent history to fill gaps in the published catalog. */
  date?: Date;
 ...
 * trending sources — the engine behind `scripts/backfill-articles.ts`. It skips
 * stages 1-2 (gather/score), synthesizes a "winner" from the topic string, and
 * runs the same research -> generate -> image -> serialize path as
 * `runPipeline`, so the output is identical in shape. Needs the LLM key and
 * `BRAVE_API_KEY` (the topic has no source URL of its own, so research depends
 * on web search) — unlike `scripts/seed-film.ts`, which writes research-free.
 */
export async function generateForTopic(
  ...
  try {
    ...
    // it so the post's sources block only lists real scraped research.
