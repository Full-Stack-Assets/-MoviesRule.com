...
hosting cost** (free tiers: Groq, Brave, Pexels, GitHub Actions, Render).
...
- **Onboarding:** niche intake → fill `site.config.ts` (branding, audience,
  sources, categories) → provision keys → deploy to Render → seed N posts → QA.
...
2. **Platform/free-tier dependency.** Groq/Brave/Pexels/Render free-tier
