...
hosting cost** (configured provider tiers plus GitHub Actions and GitHub Pages).
...
- **Onboarding:** niche intake → fill `site.config.ts` (branding, audience,
  sources, categories) → provision keys → enable GitHub Pages → seed N posts → QA.
...
2. **Platform/tier dependency.** Groq/Brave/Pexels limits and GitHub Actions
